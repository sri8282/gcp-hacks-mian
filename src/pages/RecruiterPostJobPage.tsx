...
export const RecruiterPostJobPage: React.FC = () => {
  ...
  const [company, setCompany] = useState(existingJob?.company || user?.company || 'Stripeflow Payments');
  const [companyLinkedInUrl, setCompanyLinkedInUrl] = useState(existingJob?.companyLinkedInUrl || 'https://linkedin.com/company/stripeflow-payments');
  const [category, setCategory] = useState(existingJob?.category || 'Fullstack');
  const [workplaceType, setWorkplaceType] = useState<WorkplaceType>(existingJob?.workplaceType || 'Remote');
  const [location, setLocation] = useState(existingJob?.location || 'Bengaluru, KA (Remote)');
  
  // LPA Salary
  const [minLpa, setMinLpa] = useState<number>(existingJob?.minLpa ?? 12);
  const [maxLpa, setMaxLpa] = useState<number>(existingJob?.maxLpa ?? 18);

  const [minCgpa, setMinCgpa] = useState<number>(existingJob?.minCgpa ?? 7.5);
  ...
  const [description, setDescription] = useState(
    existingJob?.description ||
      'We are looking for a high-performing engineer to build scalable software architecture, resilient transactional workflows, and elegant user-facing interfaces for enterprise systems.'
  );
  ...
  const [skills, setSkills] = useState<string[]>(
    existingJob?.skills || ['React', 'TypeScript', 'Tailwind CSS', 'Node.js', 'PostgreSQL']
  );
  const [skillsInput, setSkillsInput] = useState('');

  const [hasConfirmedRounds, setHasConfirmedRounds] = useState(
    existingJob?.interviewRounds ? existingJob.interviewRounds.length > 0 : true
  );
  const [interviewRounds, setInterviewRounds] = useState<InterviewRound[]>(
    existingJob?.interviewRounds || [
      { name: 'Round 1: Online Technical Assessment', date: '10 Sep 2026, 02:00 PM IST', format: '60-min Live Coding' },
      { name: 'Round 2: Architecture & System Design', date: '18 Sep 2026, 04:30 PM IST', format: 'Virtual Whiteboard' },
      { name: 'Round 3: Team Lead & Behavioral Discussion', date: '24 Sep 2026, 11:00 AM IST', format: '45-min Video Call' },
    ]
  );

  const [customQuestions, setCustomQuestions] = useState<string[]>(
    existingJob?.customQuestions || [
