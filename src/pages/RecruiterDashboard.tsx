...
export const RecruiterDashboard: React.FC = () => {
  ...
  const recruiterCompany = user?.company || 'Stripeflow Payments';


  // Filter jobs
  const myCompanyJobs = jobs.filter(
    (j) =>
      j.company.toLowerCase().includes(recruiterCompany.toLowerCase()) ||
      j.company.toLowerCase() === recruiterCompany.toLowerCase() ||
      recruiterCompany.toLowerCase().includes(j.company.toLowerCase())
  );
  ...
  const totalCandidateApplies = applications.length + 42;
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
          ...
          <div className="p-5 rounded-xl bg-neutral-50 dark:bg-neutral-950 border border-neutral-200 dark:border-neutral-800 text-left shadow-xs">
            ...
            <span className="text-[10px] font-mono text-neutral-500 dark:text-neutral-400 mt-1.5 block">
              18 verified applicants currently in active rounds
            </span>
          </div>
          ...
          <div className="space-y-3.5">
            {filteredJobs.length === 0 ? (
              ...
              filteredJobs.map((job) => {
                ...
                const applicantCount = jobApplies.length > 0 ? jobApplies.length : job.id === 'job-1' ? 8 : 4;
