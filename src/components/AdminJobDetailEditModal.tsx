...
export const AdminJobDetailEditModal: React.FC<AdminJobDetailEditModalProps> = ({
  ...
  useEffect(() => {
    if (job) {
      ...
      setOpenFrom(job.openFrom ? toIstDatetimeLocalString(job.openFrom) : toIstDatetimeLocalString(new Date()));
      setCloseOn(job.closeOn ? toIstDatetimeLocalString(job.closeOn) : toIstDatetimeLocalString(new Date(Date.now() + 30 * 86400000)));
      ...
      setSkills(job.skills && job.skills.length > 0 ? job.skills : ['React', 'TypeScript', 'Tailwind CSS', 'Node.js']);
      setInterviewRounds(
