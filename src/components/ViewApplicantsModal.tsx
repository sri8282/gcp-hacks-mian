...
export const ViewApplicantsModal: React.FC<ViewApplicantsModalProps> = ({ job, onClose }) => {
  ...
  // Fallback demo applicants if this is a newly created job or has few applicants
  const effectiveApplicants: JobApplication[] =
    jobApplications.length > 0
      ? jobApplications
      : [
          {
            id: `demo-app-1-${job.id}`,
            jobId: job.id,
            company: job.company,
            role: job.title,
            appliedDate: '22 Aug 2026, 02:45 PM IST',
            lastUpdatedDate: '22 Aug 2026, 02:45 PM IST',
            daysInactive: 1,
            status: 'Applied',
            notes: 'Strong candidate profile with relevant coursework and project portfolio.',
            payRange: job.payRange,
            location: job.location,
            candidateName: 'Alex Morgan',
            candidateEmail: 'alex.morgan@iitb.ac.in',
            candidateCgpa: 8.4,
            candidateCollege: 'IIT Bombay',
            candidatePassingYear: '2025',
            candidateCertifications: ['AWS Certified Cloud Practitioner', 'Meta Frontend Developer'],
            ...
            candidateSkills: ['React', 'TypeScript', 'Tailwind CSS', 'Next.js', 'GraphQL'],
            resumeFileName: 'Alex_Morgan_Resume_2026.pdf',
            answers:
              job.customQuestions && job.customQuestions.length > 0
                ? job.customQuestions.map((q) => ({
                    question: q,
                    answer: 'I have extensive experience building scalable solutions in this domain with test-driven workflows.',
                  }))
                : [
                    {
                      question: 'What is your primary experience with React performance optimization & state machines?',
                      answer: 'Built virtualized data tables rendering 100k+ rows with sub-16ms frame times and Zustand state stores.',
                    },
                  ],
          },
          {
            id: `demo-app-2-${job.id}`,
            jobId: job.id,
            company: job.company,
            role: job.title,
            ...
            daysInactive: 0,
            status: 'Interviewing',
            notes: 'Passed initial technical phone screen. Scheduled for Round 2 live architecture.',
            payRange: job.payRange,
            location: job.location,
            candidateName: 'Samantha Zhao',
            candidateEmail: 'samantha.z@pilani.bits-pilani.ac.in',
            candidateCgpa: 8.8,
            candidateCollege: 'BITS Pilani',
            candidatePassingYear: '2025',
            candidateCertifications: ['Google Cloud Professional Architect', 'Kubernetes Application Developer'],
            candidateInterestedRoles: ['Fullstack', 'Cloud Infrastructure'],
            candidateLinkedInUrl: 'https://linkedin.com/in/samanthazhao',
            candidatePortfolioUrl: 'https://samanthazhao.io',
            candidateSkills: ['React', 'TypeScript', 'Golang', 'Kubernetes', 'PostgreSQL'],
            resumeFileName: 'Samantha_Zhao_FullStack.pdf',
            answers:
              job.customQuestions && job.customQuestions.length > 0
                ? job.customQuestions.map((q) => ({
                    question: q,
                    answer: 'Led open-source projects with 1k+ GitHub stars and optimized distributed query pipelines.',
                  }))
                : [
                    {
                      question: 'What is your primary experience with React performance optimization & state machines?',
                      answer: 'Led front-end migration to React 19 concurrent features and custom Web Worker rendering pipeline.',
                    },
                  ],
          },
        ];
  ...
  const handleStatusChange = (appId: string, newStatus: ApplicationStatus) => {
    ...
    if (viewProfileApp && viewProfileApp.id === appId) {
      setViewProfileApp({ ...viewProfileApp, status: newStatus });
    }
  };

  const handleMoveToNextRound = (app: JobApplication) => {
    let nextStatus: ApplicationStatus = 'Interviewing';
    if (app.status === 'Applied') nextStatus = 'Interviewing';
    else if (app.status === 'Interviewing') nextStatus = 'Offered';
    else if (app.status === 'Offered') nextStatus = 'Offered';
    else if (app.status === 'Rejected') nextStatus = 'Applied';
    handleStatusChange(app.id, nextStatus);
  };

  const handleRejectCandidate = (app: JobApplication) => {
  ...
  const handleCheckAts = (app: JobApplication) => {
    ...
    setTimeout(() => {
      ...
      const jobSkills = job.skills || ['React', 'TypeScript', 'Tailwind CSS'];
      ...
      setAtsResult({
        ...
        grade,
        matchedKeywords: matched.length > 0 ? matched : ['React', 'TypeScript'],
            ...
            <div className="p-6 overflow-y-auto space-y-6">
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {(viewProfileApp.candidateSkills || ['React', 'TypeScript', 'Node.js', 'PostgreSQL']).map((skill) => (
                    <span
                      key={skill}
              ...
              <div className="bg-white dark:bg-black p-6 rounded-xl border border-neutral-300 dark:border-neutral-800 shadow-sm space-y-4">
                ...
                <div>
                  ...
                  <p className="text-neutral-700 dark:text-neutral-300">
                    {(previewResumeApp.candidateSkills || ['React', 'TypeScript', 'Tailwind CSS', 'Node.js', 'PostgreSQL']).join(' • ')}
                  </p>
                </div>
                ...
                <div>
                  ...
                  <p className="text-neutral-700 dark:text-neutral-300">
                    {(previewResumeApp.candidateCertifications || ['Full Stack Web Development Certification']).join(' • ')}
                  </p>
                </div>
