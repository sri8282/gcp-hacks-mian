...
export const SendNotificationModal: React.FC<SendNotificationModalProps> = ({
  ...
  const companyScope = senderRole === 'recruiter' ? (senderCompany || user?.company || 'Stripeflow Payments') : undefined;
  ...
  useEffect(() => {
    async function loadCandidates() {
      ...
      applications.forEach((app) => {
        ...
        if (isMatch) {
          const email = app.candidateEmail || 'alex.morgan@berkeley.edu';
          if (!map.has(email)) {
            map.set(email, {
              id: app.id,
