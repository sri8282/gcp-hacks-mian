...
export const SeekerDashboard: React.FC = () => {
  ...
  const [isOnboardingOpen, setIsOnboardingOpen] = useState(
    () => user?.isProfileComplete === false || (user?.seekerProfile ? !user.seekerProfile.isOnboarded : false)
  );

  // Mandatory onboarding gate effect
  useEffect(() => {
    if (user && user.role === 'seeker' && user.isProfileComplete === false) {
      setIsOnboardingOpen(true);
    }
  }, [user]);

  // Refetch fresh candidate applications & stats on mount, tab change, or window focus
  useEffect(() => {
  ...
  }, [user, activeTab, refreshCandidateStats]);
  ...
  const candidateCgpa = seekerProfile?.cgpa;
  const interestedRoles = seekerProfile?.interestedRoles || [];
  ...
  const handleDeleteApplication = async (appId: string) => {
    const confirmed = window.confirm(
      'Are you sure you want to remove this application? This cannot be undone.'
    );
    if (!confirmed) return;

    try {
      await api.applications.deleteApplication(appId);
      deleteApplication(appId);
      await refreshCandidateStats();
      showToast('Application removed successfully.');
    } catch (err: any) {
      console.error('Delete application error:', err);
      alert(err.message || 'Failed to remove application. Please try again.');
    }
  };
  ...
  const filteredJobs = useMemo(() => {
    return jobs.filter((job) => {
      ...
      if (filterMode === 'eligible') {
        if (candidateCgpa !== undefined && candidateCgpa !== null && candidateCgpa < (job.minCgpa ?? 0)) return false;
      }
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-neutral-50 dark:bg-black text-neutral-900 dark:text-white flex flex-col font-sans transition-colors">
        ...
        />
        <div className="flex-1 flex items-center justify-center font-mono p-8">
          <div className="flex flex-col items-center gap-3 p-8 rounded-2xl bg-white dark:bg-neutral-950 border border-neutral-200 dark:border-neutral-800 shadow-xl">
            <Loader2 className="w-8 h-8 text-emerald-500 animate-spin" />
            <span className="text-xs text-neutral-600 dark:text-neutral-400 font-bold">
              Loading Candidate Portal & Profile...
            </span>
          </div>
        </div>
      </div>
    ...
    return (
      <div className="min-h-screen bg-neutral-50 dark:bg-black text-neutral-900 dark:text-white flex flex-col font-sans transition-colors">
        <RoleNavbar
          ...
          onOpenProfile={() => setIsOnboardingOpen(true)}
        />
        <div className="flex-1 flex items-center justify-center font-mono p-4">
          <div className="max-w-md w-full p-6 sm:p-8 rounded-2xl bg-white dark:bg-neutral-950 border border-neutral-200 dark:border-neutral-800 shadow-xl text-center space-y-4">
            ...
            <h2 className="text-lg font-bold text-neutral-900 dark:text-white">Unable to load your profile</h2>
            <p className="text-xs text-neutral-600 dark:text-neutral-400 leading-relaxed">
              Your authentication session could not be verified — please try logging in again.
            </p>
            <button
              type="button"
              onClick={() => (window.location.href = '/login')}
              className="px-4 py-2 text-xs font-mono font-bold rounded-lg bg-neutral-900 text-white dark:bg-white dark:text-black hover:opacity-90 transition-opacity cursor-pointer inline-flex items-center gap-2"
            >
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-black text-neutral-900 dark:text-white flex flex-col font-sans transition-colors">
        ...
        <div className="mb-8 pb-6 border-b border-neutral-200 dark:border-neutral-800 flex flex-col md:flex-row md:items-center justify-between gap-4 text-left">
          <div>
            ...
            <h1 className="text-2xl sm:text-3xl font-bold tracking-tight font-heading text-neutral-900 dark:text-white">
              {getGreeting()}, {user?.name ? user.name.split(' ')[0] : 'Candidate'}
            </h1>
            <p className="text-xs font-mono text-neutral-600 dark:text-neutral-400 mt-1">
              Live automated eligibility scoring active with verified CGPA cutoff index {candidateCgpa !== undefined && candidateCgpa !== null ? candidateCgpa.toFixed(1) : 'N/A'} / 10.0 • Times displayed in Indian Standard Time (IST).
            </p>
          </div>
        ...
        {activeTab === 'notifications' ? (
          ...
          <div>
            <div className="flex items-center justify-between mb-6 text-left">
              ...
              <button
                type="button"
                onClick={() => refreshCandidateStats()}
                className="px-3 py-1.5 text-xs font-mono rounded-lg bg-neutral-100 dark:bg-neutral-900 border border-neutral-300 dark:border-neutral-800 hover:border-emerald-500 text-neutral-800 dark:text-neutral-200 flex items-center gap-1.5 transition-colors cursor-pointer"
              >
          ...
          <div>
            ...
            <div className="space-y-4 mb-6">
              ...
              <div className="flex flex-wrap items-center gap-2 text-xs font-mono">
                ...
                >
                  <CheckCircle2 className="w-3 h-3 text-emerald-500" /> Eligible Only {candidateCgpa !== undefined && candidateCgpa !== null ? `(CGPA ≥ ${candidateCgpa.toFixed(1)})` : ''}
                </button>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {alreadyApplied ? (
                            <span className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border border-emerald-500/30 text-xs font-mono font-semibold">
                              <Check className="w-3 h-3" /> {getStatusMessage(applications.find((a) => a.jobId === job.id)?.status)}
                            </span>
                            ...
                            <button
                              disabled
                              title={`Requires CGPA ${(job.minCgpa ?? 0).toFixed(1)}`}
      ...
      <SeekerOnboardingModal
        ...
        isMandatory={user?.isProfileComplete === false}
